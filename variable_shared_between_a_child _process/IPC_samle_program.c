...
int main(int argc, char *argv[]) {
    ...
    //getting minimum and maximum range of randam value input
    int min = atoi(argv[1]);
    int max = atoi(argv[2]);
    ...
    if (p1 == 0) { /* child process */
            ...
            for (i=0; i<50; i++) {
                    ...
                    *pi_child = find_factorial(*pi_child);
                    printf("Child computed factorial as: %d.\n",*pi_child);
                    getchar();
            }
            exit(0);
            ...
            for (i=0; i<50; i++) {
                     printf("Child given result as: %d\n",*pi_parent);
                    *pi_parent = rand() % (max - min + 1) + min ;
                    printf("Parent decides number as %d.\n", *pi_parent);
                    getchar();
            }
            pid = wait(&status);
